...
import dbwarden_ch_rbac

DISTRIBUTION = "dbwarden-ch-rbac"
PACKAGE = "dbwarden_ch_rbac"

# Object types this plugin promises to register. Keep in sync with setup().
OBJECT_TYPES = tuple(
    handler_class.object_type for handler_class in dbwarden_ch_rbac.HANDLER_CLASSES
)
...
def test_hook_signature_compliance() -> None:
    conformance.assert_hook_signatures(dbwarden_ch_rbac.setup)
...
def test_idempotent_setup() -> None:
    conformance.assert_idempotent_setup(dbwarden_ch_rbac.setup, plugin=DISTRIBUTION)


@pytest.mark.parametrize("handler_class", dbwarden_ch_rbac.HANDLER_CLASSES)
